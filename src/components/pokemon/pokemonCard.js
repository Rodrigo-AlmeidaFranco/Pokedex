...
import spiner from '../pokemon/load.gif'
...
export default class pokemonCard extends Component {
    ...
    render() { 
        return (
            ...
            {this.state.imgLoadin?(
                <img src={spiner} style={{width: '5em',height:'5em'}}
                className="card-img-top rounded mx-auto d-block mt-2"></img>
            ):null}
            <Sprite
                className="card-img-top rounded mx-auto mt-2"
                onLoad={()=>this.setState({imgLoadin:false})}
                onError={()=>this.setState({toManyRequest: true})}
                src={this.state.imgUrl}
                style={
                    this.state.toManyRequest?{display:"none"}:
                    this.state.imgLoadin? null: {display:"block"}
                }
            />
            {this.toManyRequest?(<h6 className="mx-auto">
